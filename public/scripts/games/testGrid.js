    ...
    eventBus.on("new frame", function (){
        ...
        {    
            ...
            for(var i = 0; i < test.grid.caseTable.length; i++)
            {
                ...
                {
                    if(test.mouse.x >= test.grid.caseTable[i][j].x && test.mouse.x <= (test.grid.caseTable[i][j].x + test.grid.caseTable[i][j].width)
                        && test.mouse.y >= test.grid.caseTable[i][j].y && test.mouse.y <= (test.grid.caseTable[i][j].y + test.grid.caseTable[i][j].height))
                    {
                        test.grid.caseTable[i][j].select(true);
                    }
                    else
                        test.grid.caseTable[i][j].select(false);
                }
            }
        }
    });
