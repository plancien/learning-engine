            ...
            eventBus.on("images loaded", function (images){
                ...
                var Player = function (params){
                    ...
                    eventBus.emit('init render', {
                        object : this,
                        sprite : {x : 0, y : 0, width : 25, height : 25, img : images["frog"]},
                        rotating : true
                    });
                    ...
                    eventBus.on("key pressed", function (keycode){
                        ...
                        canMove = false;
                        moveDirection = {x:x,y:y};
                        eventBus.emit("play animation", player, "jump");
                    })

                    eventBus.on("collision", function(parama, paramb){console.log(parama, paramb);})
                }
                var Strip = function (params){
                    ...
                    eventBus.emit("init render",{
                        object : this,
                        sprite : {x : 0, y : 0, width : 32, height : 32, img : images[this.type]},
                ...
                var Car = function (params){
                    ...
                    eventBus.emit("init render",{
                        object : this,
                        sprite : {x : this.sourceX, y : 0, width : 48, height : 48, img : images.cars},
                        rotating : true
                    });
                ...
                var Bonus = function (params){
                    ...
                    eventBus.emit("init render",{
                        object : this,
                        sprite : {x : 0, y : 0, width : this.image.width, height : this.image.height, img : this.image}
                    });
                }
                ...
                var cars = [];
                var bonuses = [];
                strips.push(new Strip({y : 600-37.5, type : "grass"}));
                generateStrips(0, strips, cars, bonuses);
                var player = new Player({x:400, y : 600-37.5, width : 60, height : 60});
                ...
                eventBus.on("new frame", function (){
                    ...
                    if(player.y <= 75/2){
                        ...
                        bonuses = [];
                        cars = [];
                        strips.push(new Strip({y : 600-37.5 - 600, type : "grass"}));
                ...
                function generateStrips (offset, strips, cars, bonuses){
                    ...
                    strips.push(new Strip({y : 37.5 + offset, type : "grass"}));
                    for(var i = 1; i < 7; i++){
                        if(pausePosition === i){
                            strips.push(new Strip({y : 37.5 + pausePosition * 75 + offset, type : "grass"}));
                            continue;
                        }
                        var strip = new Strip({y : 37.5 + i * 75 + offset, type : "road", direction : direction})
                        strips.push(strip);
                        ...
                        for(var j = 0; j < strip.carsNumber; j++){
                            var car = new Car({y : strip.y, x : chosenPattern[j], speed : strip.carSpeed, direction : strip.direction})
                            cars.push(car);
                        }
                    }
                ...
                function generateBonuses (offset, bonuses){
                    ...
                    var bonus = new Bonus({x : x, y : y + offset, good : true});
                    bonuses.push(bonus);
                    x = 75/2 + Math.round(Math.random() * 10) * 75;
                    y = 75/2 + Math.round(Math.random() * 8) * 75;
                    var bonus = new Bonus({x : x, y : y + offset, good : false});
                    bonuses.push(bonus);
                    for(var i = 0; i < moreBonusesNumber; i++){
                        x = 75/2 + Math.round(Math.random() * 10) * 75;
                        y = 75/2 + Math.round(Math.random() * 8) * 75;
                        var good = Math.round(Math.random());
                        var bonus = new Bonus({x : x, y : y + offset, good : good});
                        bonuses.push(bonus);
                    }
                }
            });
